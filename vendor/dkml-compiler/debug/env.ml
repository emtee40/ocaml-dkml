...
(*                                                                        *)
(**************************************************************************)
[@@@warning "-32"] (* 32=unused-value-declaration *)
...
let print_path =
  ref ((fun _ _ -> assert false) : Format.formatter -> Path.t -> unit)

let print_longident =
  ref ((fun _ _ -> assert false) : Format.formatter -> Longident.t -> unit)

let report_lookup_error _loc _env ppf =
  let open Format in
  function
...
let error err =
  ...
  | Illegal_value_name(_loc, name) ->
      fprintf ppf "'%s' is not a valid value identifier."
        name
  | Lookup_error(loc, t, err) ->
      fprintf ppf "Lookup_error(loc=%a; %a)" (Location.print_loc) loc
      (report_lookup_error loc t) err
  in
  (match Sys.getenv_opt "OPAM_PACKAGE_NAME" with (* Some packages fail to build so by default skip 'opam install' *)
  | Some pkg when String.length pkg > 0 -> ()
  | _ -> eprintf "[typing/env.ml] raise (Env.Error (%a))@." report_error err);
  raise (Error err)
