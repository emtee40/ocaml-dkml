...
let probe_os_path_sep =
  ...
  lazy
    (Result.map
       (fun path ->
         match
           ( Astring.String.find (fun c -> c = ';') path,
             Astring.String.find (fun c -> c = ':') path )
         with
...
let prune_envvar ~f ~path_sep varname =
  ...
  if "" = varvalue then Ok ()
    ...
    Astring.String.cuts ~empty:false ~sep:path_sep varvalue |> List.filter f
    |> fun entries ->
    Some (String.concat path_sep entries) |> OS.Env.set_var varname
...
let prepend_envvar ~path_sep varname dir =
  let open Bos in
  function
...
let prepend_entries ~tools installed_dir =
  ...
  (if tools then
     ...
     >>= fun matches ->
     Ok (List.map (fun (_fp, pat) -> Astring.String.Map.get "tool" pat) matches)
     >>= fun tools_with_exe ->
...
let set_tempvar_entries cache_keys =
  ...
  let set_windows_tmp tmpdir =
    ...
    Ok (dos83path :: dos83path :: cache_keys)
  in
  Lazy.force Dkml_context.get_msys2_dir_opt >>= function
  | None ->
...
let prune_path_of_microsoft_visual_studio () =
  ...
  |> List.filter (fun entry ->
         let contains = Dkml_environment.path_contains entry in
         let ends_with = Dkml_environment.path_ends_with entry in
         not
  ...
  |> fun paths ->
  Some (Astring.String.concat ~sep:";" paths) |> OS.Env.set_var "PATH"
...
let remove_microsoft_visual_studio_entries () =
  ...
  Astring.String.Map.iter
    (fun varname _varvalue ->
      if Astring.String.is_prefix ~affix:"VSCMD_" varname then
...
let null_possible_dir dir =
  let open Bos in
  match Fpath.of_string dir with Error _ -> OS.File.null | Ok dir -> dir

let prune_3p_entry ~threep entry =
  let fp_opt = Fpath.of_string entry in
  match fp_opt with
  | Error _ -> false
  | Ok fp -> not Fpath.(is_prefix threep fp)
...
let set_3p_prefix_entries cache_keys =
  ...
  let rec helper = function
    | [] -> Ok ()
    | dir :: rst ->
        let null_possible_dir = null_possible_dir dir in
        if Fpath.compare OS.File.null null_possible_dir = 0 then
          ...
          (* 1. Remove 3p entries, if any, from compiler variables and PKG_CONFIG_PATH.
              ...
              well.
          *)
          prune_entries (prune_3p_entry ~threep) >>= fun () ->
  ...
  Astring.String.concat ~sep:";" dirs :: cache_keys
  ...
  let rec helper = function
    ...
    | dir :: rst ->
        ...
        if Fpath.compare OS.File.null null_possible_dir = 0 then
          ...
          let threep = null_possible_dir in
          let f = prune_3p_entry ~threep in
...
let set_msvc_entries cache_keys =
  ...
  *)
  ...
  let* vsstudio_dir_opt = Lazy.force Dkml_context.get_vsstudio_dir_opt in
  ...
  | Some msys2_dir -> (
      let do_set setvars =
        List.iter
          ...
          (Dkml_context.association_list_of_sexp setvars)
      in
      Lazy.force Opam_context.get_opam_switch_prefix
      ...
      let cache_key =
        ...
        let ctx =
          List.fold_left
            (fun ctx' key -> Digestif.SHA256.feed_string ctx' key)
            ctx cache_keys
        in
        Digestif.SHA256.(get ctx |> to_hex)
      in
      ...
      if cache_hit then (
        ...
        let setvars = Sexplib.Sexp.load_sexp (Fpath.to_string cache_file) in
        do_set setvars;
        Ok cache_keys)
      else
        (* Cache miss *)
        let cache_miss tmpdir () =
          ...
          let env_vars =
            Sexp.load_sexp_conv_exn
              ...
              Dkml_context.association_list_of_sexp
          in
          Logs.debug (fun l ->
...
let do_use ~(mode : [ `Direct | `WithDkml ]) ~argv ~extract_dkml_scripts
  ...
  *)
  let* has_dkml_mutating_ancestor_process =
    Dkml_environment.mark_dkml_mutating_ancestor_process ()
  in
  ...
  if not has_dkml_mutating_ancestor_process then
    Dkml_news.show_and_update_if_expired `Initialized;
  ...
  *)
  ...
  let* () =
    Lazy.force Dkml_context.get_dkmlhome_dir_opt >>| function
    | None -> ()
    | Some dkmlhome_dir ->
