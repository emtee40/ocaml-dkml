...
* Once every two weeks DkML news about new versions, errata, uninstalling, etc. will be shown on a webpage. It is triggered from `dkml init` and the `with-dkml` proxy commands, and can be disabled with `dkml news disable`. In particular, use `dkml news` to show the news if you are experiencing problems with DkML.
